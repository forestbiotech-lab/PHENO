...
def CheckInputFile(input_file, logs):

    if input_file.lower().endswith(('.xlsx', '.xls', 'ods')):
        logs.append("CHECK PASSED - Valid input file extension")
    else:
        logs.append("CHECK FAILED - Invalid input file extension")
        sys.exit(" - ERROR - Invalid Input File Format / Extension - ")

    # Check sheet names and sheet number
    complete_excel = pd.ExcelFile(input_file)
    sheetsList = complete_excel.sheet_names

    # These are all valid sheet names for a MIAPPE compliant excel file
    valid_sheet_names = ["Investigation", "Study", "Person", "Data file", "Biological Material", "Sample", "Observation Unit", "Environment", "Factor", "Observed Variable", "Event"]

    if set(sheetsList) <= set(valid_sheet_names):
        if len(sheetsList) == 11:
            logs.append("CHECK PASSED - The " + str(len(sheetsList)) + " sheet names of the excel input file are valid.")
        else:
            logs.append("CHECK WARNING - The input file does not have 11 sheets.")
    else:
        logs.append("CHECK FAILED - The input file contains invalid sheet names.")
...
def CheckInvestigationSheet(input_file, logs):

    # Check Investigation Sheet Header
    sheet_df = pd.read_excel(input_file, 'Investigation')
    # Remove '*' characters, which indicate mandatory columns to fill
    investigation_header = [ele.replace('*', '') for ele in list(sheet_df)]

    valid_investigation_header = ["Investigation unique ID", "Investigation title", "Investigation description", "Submission date", "Public release date", "License", "MIAPPE version", "Associated publication"]

    if investigation_header == valid_investigation_header:
        logs.append("CHECK PASSED - The Investigation sheet has a valid header (column name/number).")
    else:
        logs.append("CHECK FAILED - The Investigation sheet has an invalid header (column name/number).")
    ...
    investigation_format = sheet_df.dtypes
    ...
    if str(list(investigation_format)) == valid_investigation_format:
        logs.append("CHECK PASSED - The Investigation sheet has a valid format (properly formated fields).")
    else:
        logs.append("CHECK FAILED - The Investigation sheet has a invalid format (some fields are incorrectly formated).")
        sys.exit(" - ERROR - Invalid Field Formats in Investigation Sheet - ")

    # Check if the Investigation unique ID holds unique values

    investigation_unid_col = sheet_df.loc[:,"Investigation unique ID"].is_unique
    if investigation_unid_col == True:
        logs.append("CHECK PASSED - The Investigation sheet has no duplicate Investigation unique IDs.")
    else:
        logs.append("CHECK FAILED - The Investigation sheet has duplicate Investigation unique IDs (they should be unique).")
...
def CheckStudySheet(input_file, logs):
    ...
    valid_study_header1 = ["Study unique ID", "Study title", "Study description", "Start date of study", "End date of study", "Contact institution", "Geographic location (country)", "Experimental site name", "Geographic location (latitude)", "Geographic location (longitude)", "Geographic location (altitude)", "Description of the experimental design", "Type of experimental design", "Observation unit level hierarchy", "Observation unit description", "Description of growth facility", "Type of growth facility", "Cultural practices", "Map of experimental design"]
    valid_study_header2 = ["Investigation unique ID", "Study unique ID", "Study title", "Study description", "Start date of study", "End date of study", "Contact institution", "Geographic location (country)", "Experimental site name", "Geographic location (latitude)", "Geographic location (longitude)", "Geographic location (altitude)", "Description of statistical design", "Type of statistical design", "Observation unit level hierarchy", "Observation unit description", "Description of growth facility", "Type of growth facility", "Cultural practices", "Map of experimental design"]
...
def CheckPersonSheet(input_file, logs):
    ...
    valid_person_header2 = ["Study unique ID", "Person name", "Person email", "Person ID", "Person role", "Person affiliation"]
...
def CheckBiologicalMaterialSheet(input_file, logs):
    ...
    valid_biomaterial_header1 = ["Biological material ID", "Organism", "Genus", "Species", "Infraspecific name", "Biological material latitude", "Biological material longitude", "Biological material altitude", "Biological material coordinates uncertainty", "Biological material preprocessing", "Material source ID (Holding institute/stock centre, accession)", "Material source DOI", "Material source latitude", "Material source longitude", "Material source altitude", "Material source coordinates uncertainty", "Material source description"]
    valid_biomaterial_header2 = ["Study unique ID", "Biological material ID", "Organism", "Genus", "Species", "Biological material latitude", "Biological material longitude", "Biological material altitude", "Biological material coordinates uncertainty", "Biological material preprocessing", "Material source ID", "Material source DOI", "Material source latitude", "Material source longitude", "Material source altitude", "Material source coordinates uncertainty", "Material source description"]
    ...
    if str(list(biomaterial_format)) == valid_biomaterial_format:
        ...
        logs.append("CHECK FAILED - The Biological Material sheet has a invalid format (some fields are incorrectly formated).")
...
def CheckEventSheet(input_file, logs):
    ...
    valid_event_header2 = ["Study unique ID", "Observation unit ID", "Event type", "Event acession number", "Event description", "Event date"]
...
def CheckObservationUnitSheet(input_file, logs):
    ...
    valid_obsunit_header1 = ["Observation unit ID", "Observation unit type", "External ID", "Spatial distribution", "Observation Unit factor value"]
    valid_obsunit_header2 = ["Study unique ID", "Biological Material ID", "Observation unit ID", "Observation unit type", "External ID", "Spatial distribution", "Observation unit factor value"]
...
def CheckSampleSheet(input_file, logs):
    ...
    valid_sample_header1 = ["Sample ID", "Plant structure development stage", "Plant anatomical entity", "Sample description", "Collection date", "External ID"]
    valid_sample_header2 = ["Observation unit ID", "Sample ID", "Plant structure development stage", "Plant anatomical entity", "Sample description", "Collection date", "External ID"]
...
def CheckObservedVariableSheet(input_file, logs):
    ...
    valid_obsvariable_header1 = ["Variable ID", "Variable name", "Variable accession number", "Trait", "Trait accession number", "Method", "Method accession number", "Method description", "Reference associated to the method", "Scale", "Scale accession number", "Time scale"]
    valid_obsvariable_header2 = ["Study unique ID", "Variable ID", "Variable name", "Variable accession number", "Trait", "Trait accession number", "Method", "Method accession number", "Method description", "Reference associated to the method", "Scale", "Scale accession number", "Time scale"]
...
logs = ["   --- OntoBrAPI - Input File Validity Report ---   "]
...
CheckInputFile(input_file, logs)

CheckInvestigationSheet(input_file, logs)
CheckStudySheet(input_file, logs)
CheckPersonSheet(input_file, logs)
CheckDatafileSheet(input_file, logs)
CheckBiologicalMaterialSheet(input_file, logs)
CheckEnvironmentSheet(input_file, logs)
CheckExperimentalFactorSheet(input_file, logs)
CheckEventSheet(input_file, logs)
CheckObservationUnitSheet(input_file, logs)
CheckSampleSheet(input_file, logs)

# Write miappe_validator_logs file:

logs.append(" - THE INPUT FILE IS VALID - ")

with open(r'/mnt/c/Users/TRACE_Rice/Documents/Projects/OntoBrAPI/miappe_validator_logs.txt', 'w') as log:
    for item in logs:
        log.write("%s\n" % item)
